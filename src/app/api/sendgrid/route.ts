...
export async function POST(request: Request) {
  ...
  try {
    await fetch(process.env.MAIL_API_URL as string, options);
    return NextResponse.json( { success: true });
  }
  catch (error) {
    const fetchError = error as FetchError;
    return NextResponse.json({errors: {root: {message: fetchError.message || 'An unknown error has occurred'}}}, {status: fetchError.statusCode || 500});
  }
}
